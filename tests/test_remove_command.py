...
def test_remove_noninteractive_requires_yes() -> None:
    ...
    with patch("sfutils_pat.pat.sys.stdin", stdin):
...
def test_remove_pat_only_yes_runs_without_prompts() -> None:
    ...
    stdin.isatty.return_value = False
    with (
        patch("sfutils_pat.pat.sys.stdin", stdin),
        patch(
            "sfutils_pat.pat.get_snowflake_connection_metadata",
            return_value=("ACC", "host.example"),
        ),
        patch("sfutils_pat.pat.delete_pat"),
        patch("sfutils_pat.pat.remove_pat"),
        patch("sfutils_pat.pat.clear_env"),
    ):
...
def test_remove_yes_lists_network_objects() -> None:
    ...
    with (
        ...
        patch("sfutils_pat.pat.delete_pat"),
        patch("sfutils_pat.pat.remove_pat"),
        patch("sfutils_pat.pat.cleanup_network_for_user"),
        patch("sfutils_pat.pat.remove_auth_policy"),
        patch("sfutils_pat.pat.clear_env"),
    ):
        result = runner.invoke(
            cli,
